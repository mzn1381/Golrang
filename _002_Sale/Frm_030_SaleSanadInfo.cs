        ...
        public Frm_030_SaleSanadInfo(DataRowView saleRow)
        {
            ...
            try
            {
                ...
                Adapter = new SqlDataAdapter(
                                                @"SELECT     dbo.Table_024_Discount.column10, dbo.Table_024_Discount.column16, dbo.Table_012_Child2_SaleFactor.column01
                FROM         dbo.Table_024_Discount INNER JOIN
                                      dbo.Table_012_Child2_SaleFactor ON dbo.Table_024_Discount.columnid = dbo.Table_012_Child2_SaleFactor.column02
                WHERE     (dbo.Table_024_Discount.column02 = 1) AND (dbo.Table_012_Child2_SaleFactor.column01 = " + int.Parse(SaleRow["ColumnId"].ToString()) + ") ", ConSale);
                setting2 = new DataTable();
                Adapter.Fill(setting2);

                if (setting2.Rows.Count > null)
                {
                ...
                Adapter = new SqlDataAdapter(@"SELECT     dbo.Table_024_Discount.column16, dbo.Table_024_Discount.column10, dbo.Table_012_Child2_SaleFactor.column01
FROM         dbo.Table_024_Discount INNER JOIN
                      dbo.Table_012_Child2_SaleFactor ON dbo.Table_024_Discount.columnid = dbo.Table_012_Child2_SaleFactor.column02
WHERE     (dbo.Table_024_Discount.column02 = 0) AND (dbo.Table_012_Child2_SaleFactor.column01 = " + int.Parse(SaleRow["ColumnId"].ToString()) + ") ", ConSale);
                setting3 = new DataTable();
                Adapter.Fill(setting3);
                if (setting3.Rows.Count > null)
                {
...
//                        }
                       ...
                       int HavaleNum = clDoc.MaxNumber(ConWare.ConnectionString, "Table_007_PwhrsDraft", "Column01");
                        ...
                        using (SqlConnection Con = new SqlConnection(Properties.Settings.Default.PWHRS))
                        {
                            ...
                            SqlCommand InsertHeader = new SqlCommand(@"INSERT INTO Table_007_PwhrsDraft ([column01]
                                ...
                                "',0,'" + Class_BasicOperation._UserName + "',getdate(),'" + Class_BasicOperation._UserName + "',getdate(),0,NULL,NULL,0," + SaleRow["ColumnId"].ToString() + ",0,0,0,0,0,0,0,null,0,1); SET @Key=SCOPE_IDENTITY()", Con);
           ...
           ,[Column33]
           ,[Column34]
           ,[Column35],[Column36],[Column37]) VALUES(" + HavaleID + "," + item1["Column02"].ToString() + "," +
                                       clDoc.ExScalar(ConWare.ConnectionString,
                                        "table_004_CommodityAndIngredients", "column07", "ColumnId",item1["Column02"].ToString()) + "," + item1["Column04"].ToString() + "," + item1["Column05"].ToString() + "," + item1["Column07"].ToString() + "," +
                                        item1["Column07"].ToString() + "," + item1["Column08"].ToString() + "," + item1["Column09"].ToString() + "," + item1["Column10"].ToString() + "," +
                                        item1["Column11"].ToString() + ",N'"+ item1["Column23"].ToString()+"',NULL," + (item1["Column22"].ToString().Trim() == "" ? "NULL" : item1["Column22"].ToString())
                                        ...
                                        item1["Column15"].ToString() +
                                            ...
                                            + "," + item1["Column32"].ToString() + "," + item1["Column36"].ToString() + "," + item1["Column37"].ToString() + ",N'" + item1["Column38"].ToString()  + "',N'" +item1["Column39"].ToString()+"' )", Con);
