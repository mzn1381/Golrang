...
{
    public partial class Frm_04_Recipt_checks : Form
    {
        ...
        public Frm_04_Recipt_checks()
        {
            InitializeComponent();
        }

        private void Frm_04_Recipt_checks_Load(object sender, EventArgs e)
        {
            try
            {

            //string status = ClDoc.ExScalar(ConBank.ConnectionString, @"select isnull((select ColumnId from Table_060_ChequeStatus where Column15=4),0)");
            //mlt_status.DataSource = ClDoc.ReturnTable(ConBank, @"select ColumnID,Column01,Column02 from Table_060_ChequeStatus where Column15=4");
            ...
            mlt_status.DataSource = ClDoc.ReturnTable(ConBank, @"select * from Table_060_ChequeStatus where Column15=4");
...
//                      dbo.Table_035_ReceiptCheques.Column55, Table_065_TurnReception.Column01 AS DocId, dbo.Table_060_ChequeStatus.Column15 AS Status, 
//                      Table_065_TurnReception.Column19 AS TurnDes, Table_065_TurnReception.Column21, Table_065_TurnReception.Column20, Table_065_TurnReception.Column02 AS gardesh, 
//                      Table_065_TurnReception.Column22, Table_065_TurnReception.Column06 AS TurnBankBox, Table_065_TurnReception.Column07 AS TurnACC, 
//                      dbo.Table_010_BankNames.Column01 AS MainBankName, Table_065_TurnReception.Column13 AS TurnSanad, dbo.Table_020_BankCashAccInfo.Column12 AS Bed
...
//                         dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId INNeR JOIN
//						   dbo.Table_010_BankNames ON dbo.Table_035_ReceiptCheques.Column08 = dbo.Table_010_BankNames.Column00 
//WHERE     (dbo.Table_060_ChequeStatus.Column15 IN (3,1)) AND (dbo.Table_065_TurnReception.Column13 <> 0) AND (dbo.Table_065_TurnReception.ColumnId IN
...
//                            FROM          dbo.Table_065_TurnReception AS Table_065_TurnReception_1
//                            WHERE      (Column13 <> 0)
//                            GROUP BY Column01)) AND (dbo.Table_035_ReceiptCheques.Column42 = N'" + Class_BasicOperation._UserName + "') AND (Table_065_TurnReception.Column16 = N'" + Class_BasicOperation._UserName + "')");

            //mlt_status_ValueChanged(sender, e);
            ...
            try
            {
                ...
                if (gridEX1.GetCheckedRows().Length > 0)
                {
                    ...
                    if (MessageBox.Show("آیا مایل به صدور سند حسابداری هستید؟", "توجه", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        string BedPerson = (gridEX1.GetValue("Column46").ToString());
                        CommandText = "";
                        foreach (GridEXRow item in gridEX1.GetCheckedRows())
                        {
                            if (item.Cells["Status"].Value.ToString() == "3")
                            {
                            ...
                            else
                            {
                                ...
                                /// if (CheckNotExported(int.Parse(item.Cells["ChqID"].Value.ToString()), item.Cells["LastState"].Value.ToString()))
                                {
                                    ...
                                    CommandText = CommandText + AddTurnReception(int.Parse(item.Cells["ColumnID"].Value.ToString()), short.Parse(mlt_status.Value.ToString()), faDatePicker1.Text,
                                         BedPerson, ("NULL"),
                                                      mlt_Bed.Value.ToString().TrimEnd(), Int16.Parse(_Bed_Info[0].ToString()),
                        ...
                        mlt_Bed.Value = null;
                        mlt_Bes.Value = null;
                        //Frm_04_Recipt_checks_Load(sender, e);
                        mlt_status_ValueChanged(sender, e);
                    }
...
int SanadID, string Center, string Project, string User, string Description, string Currency, string CurrencyType, Double CurrencyValue)
        ...
        private void btn_Save1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                    //if (MessageBox.Show("آیا مایل به صدور سند حسابداری هستید؟", "توجه", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    //{
               DataTable ACCBAnkINFo = ClDoc.ReturnTable(ConBank, @"SELECT        Column12
FROM            dbo.Table_020_BankCashAccInfo
WHERE        (ColumnId = " + gridEX1.GetValue("TurnBankBox") + ")");
                string[] _Bed_Info = ClDoc.ACC_Info(ACCBAnkINFo.Rows[0][0].ToString());
                        string Bes = (mlt_Bes.Text.Trim() == "" ? "NULL" : mlt_Bes.Value.ToString());
                        ...
                        foreach (GridEXRow item in gridEX1.GetCheckedRows())
                        {
                            ...
                            /// if (CheckNotExported(int.Parse(item.Cells["ChqID"].Value.ToString()), item.Cells["LastState"].Value.ToString()))
                            {
                                ...
                                //if (chk_BesPerson.Checked && item.Cells["TurnPerson"].Text.Trim() != "")
                                //    BesPerson = item.Cells["TurnPerson"].Value.ToString();
                                DataTable ACCBAnk = ClDoc.ReturnTable(ConBank, @"SELECT        Column12
FROM            dbo.Table_020_BankCashAccInfo
WHERE        (ColumnId = " + item.Cells["TurnBankBox"].Value + ")");


                                //*******First Add Turn to TurnTable***************//
                                CommandText = CommandText + AddTurnReception(int.Parse(item.Cells["ColumnID"].Value.ToString()), short.Parse(mlt_status.Value.ToString()), faDatePicker1.Text,
                                     BedPerson, ("NULL"),
                                      ACCBAnk.Rows[0][0].ToString().TrimEnd(), Int16.Parse(_Bed_Info[0].ToString()),
                                      (_Bed_Info[1].ToString() == "" ? "NULL" : _Bed_Info[1].ToString()), (_Bed_Info[2].ToString() == string.Empty ? "NULL" : _Bed_Info[2].ToString())
                                      , (_Bed_Info[3].ToString() == "" ? "NULL" : _Bed_Info[3].ToString()), (_Bed_Info[4].ToString() == string.Empty ? "NULL" : _Bed_Info[4].ToString()), DocID, ("NULL"),
                                     BedProject, Class_BasicOperation._UserName,
                                      (item.Cells["TurnDes"].Text.Trim() == "" ? "" : item.Cells["TurnDes"].Text.Trim()),
                                ...
                                if (item.Cells["Column20"].Value.ToString() == "False")
                                {
                                    CommandText = CommandText + ExportDoc_Detail(DocID, ACCBAnk.Rows[0][0].ToString(), Int16.Parse(_Bed_Info[0].ToString()),
                                    _Bed_Info[1].ToString(), _Bed_Info[2].ToString(), _Bed_Info[3].ToString(),
                                    _Bed_Info[4].ToString(), (BedPerson == "" ? "NULL" : BedPerson),
                                    (null), BedProject,
                                        //(mlt_BedProject.Text.Trim() == "" ? null : mlt_BedProject.Value.ToString()),
    ...
    Int64 Bes, Double CurBed, Double CurBes, Int16 CurType, Int16 SanadType, int Ref, string RegUser, Double CurPrice, object Period, object EffDate)
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column44", 101))
                {
...
//                SqlDataAdapter dtreciptcheq= new SqlDataAdapter(@"SELECT     dbo.Table_035_ReceiptCheques.ColumnId, dbo.Table_035_ReceiptCheques.Column00, dbo.Table_035_ReceiptCheques.Column01, dbo.Table_035_ReceiptCheques.Column02, 
//                      dbo.Table_035_ReceiptCheques.Column03, dbo.Table_035_ReceiptCheques.Column04, dbo.Table_035_ReceiptCheques.Column05, dbo.Table_035_ReceiptCheques.Column06, 
//                      dbo.Table_035_ReceiptCheques.Column07, dbo.Table_035_ReceiptCheques.Column08, dbo.Table_035_ReceiptCheques.Column09, dbo.Table_035_ReceiptCheques.Column10, 
//                      dbo.Table_035_ReceiptCheques.Column11, dbo.Table_035_ReceiptCheques.Column12, dbo.Table_035_ReceiptCheques.Column15, dbo.Table_035_ReceiptCheques.Column41, 
//                      dbo.Table_035_ReceiptCheques.Column42, dbo.Table_035_ReceiptCheques.Column43, dbo.Table_035_ReceiptCheques.Column44, dbo.Table_035_ReceiptCheques.Column45, 
...
//                         dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId INNeR JOIN
//						   dbo.Table_010_BankNames ON dbo.Table_035_ReceiptCheques.Column08 = dbo.Table_010_BankNames.Column00 
//                    WHERE     (dbo.Table_060_ChequeStatus.Column15 IN (1,3)) AND (dbo.Table_065_TurnReception.Column13 <> 0) AND (dbo.Table_065_TurnReception.ColumnId IN
//                          (SELECT     MAX(ColumnId) AS Expr1
//                            FROM          dbo.Table_065_TurnReception AS Table_065_TurnReception_1
//                            WHERE      (Column13 <> 0)
//                            GROUP BY Column01))",ConBank);
//                dtreciptcheq.Fill(dt);
            //dt = ClDoc.ReturnTable(ConBank, );

            //foreach (DataRow row in dt.Rows)
            //{
            if (((DataRowView)mlt_Bes.DropDownList.FindItem(mlt_Bes.Value))["ACC_Code"].ToString()=="104100")
                {
                    ...
                    FROM            dbo.Table_065_TurnReception INNER JOIN
                         ...
                         dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId INNeR JOIN
						   dbo.Table_010_BankNames ON dbo.Table_035_ReceiptCheques.Column08 = dbo.Table_010_BankNames.Column00 
                WHERE     (dbo.Table_060_ChequeStatus.Column15 IN (3)) AND (dbo.Table_065_TurnReception.Column13 <> 0) AND (dbo.Table_065_TurnReception.ColumnId IN
                          (SELECT     MAX(ColumnId) AS Expr1
                            FROM          dbo.Table_065_TurnReception AS Table_065_TurnReception_1
                            WHERE      (Column13 <> 0)
                            GROUP BY Column01))");
...
FROM            dbo.Table_065_TurnReception INNER JOIN
                         ...
                         dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId INNeR JOIN
						   dbo.Table_010_BankNames ON dbo.Table_035_ReceiptCheques.Column08 = dbo.Table_010_BankNames.Column00 
WHERE     (dbo.Table_060_ChequeStatus.Column15 IN (1)) AND (dbo.Table_065_TurnReception.Column13 <> 0) AND (dbo.Table_065_TurnReception.ColumnId IN
                          (SELECT     MAX(ColumnId) AS Expr1
                            FROM          dbo.Table_065_TurnReception AS Table_065_TurnReception_1
                            WHERE      (Column13 <> 0)
                            GROUP BY Column01))");
