...
{
    public partial class Frm_05_Return_checks : Form
    {
        ...
        public Frm_05_Return_checks()
        {
            InitializeComponent();
        }

        private void Frm_05_Return_checks_Load(object sender, EventArgs e)
        {
            ...
            string status = ClDoc.ExScalar(ConBank.ConnectionString, @"select isnull((select ColumnId from Table_060_ChequeStatus where Column15=5 AND Column09=104100),0)");
            //mlt_Bed.DataSource = ClDoc.ReturnTable(ConACNT, "select  * from AllHeaders() where ACC_Code=103100");
            //mlt_Bes.DataSource = ClDoc.ReturnTable(ConACNT, "select  * from AllHeaders() where ACC_Code= 104100 ");
            //mlt_Bed.Value = Bed;
            //mlt_Bes.Value = Bes;
            ...
            gridEX1.DropDowns["ToBank"].DataSource = ClDoc.ReturnTable(ConBank, @"select * from Table_020_BankCashAccInfo");
            ...
            if (isadmin)
            {
                gridEX1.DataSource = ClDoc.ReturnTable(ConBank, @"SELECT     dbo.Table_035_ReceiptCheques.ColumnId, dbo.Table_035_ReceiptCheques.Column00, dbo.Table_035_ReceiptCheques.Column01, dbo.Table_035_ReceiptCheques.Column02, 
                      ...
                      Table_065_TurnReception_2.Column22, Table_065_TurnReception_2.Column06 AS TurnBankBox, Table_065_TurnReception_2.Column07 AS TurnACC, 
                      dbo.Table_010_BankNames.Column01 AS MainBankName, Table_065_TurnReception_2.Column13 AS TurnSanad, dbo.Table_020_BankCashAccInfo.Column12 AS Bed
                      ...
                      dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId
                        WHERE     (TurnReception.LastState IN
                          (SELECT     ColumnId
                            ...
                            WHERE      (Column15 = 3))) AND (Table_065_TurnReception_2.Column01 <> 0) AND (dbo.Table_060_ChequeStatus.Column15 = 3)");
            }
            ...
            else
            {
                gridEX1.DataSource = ClDoc.ReturnTable(ConBank, @"SELECT     dbo.Table_035_ReceiptCheques.ColumnId, dbo.Table_035_ReceiptCheques.Column00, dbo.Table_035_ReceiptCheques.Column01, dbo.Table_035_ReceiptCheques.Column02, 
                      ...
                      Table_065_TurnReception_2.Column19 AS TurnDes, Table_065_TurnReception_2.Column21, Table_065_TurnReception_2.Column20, Table_065_TurnReception_2.Column02 AS gardesh, 
                      Table_065_TurnReception_2.Column22, Table_065_TurnReception_2.Column06 AS TurnBankBox, Table_065_TurnReception_2.Column07 AS TurnACC, 
                      dbo.Table_010_BankNames.Column01 AS MainBankName, Table_065_TurnReception_2.Column13 AS TurnSanad, dbo.Table_020_BankCashAccInfo.Column12 AS Bed,Table_065_TurnReception_2.Column16
                    FROM         dbo.Table_035_ReceiptCheques INNER JOIN
                          (SELECT     TOP (100) PERCENT Column01 AS PaperID, ColumnId AS LastTurnID, Column02 AS LastState
                            FROM          dbo.Table_065_TurnReception
                            WHERE      (Column13 <> 0) AND (ColumnId IN
                      ...
                      dbo.Table_020_BankCashAccInfo ON dbo.Table_035_ReceiptCheques.Column01 = dbo.Table_020_BankCashAccInfo.ColumnId
                        WHERE     (TurnReception.LastState IN
                          (SELECT     ColumnId
                            FROM          dbo.Table_060_ChequeStatus AS Table_060_ChequeStatus_1
                            WHERE      (Column15 = 3))) AND (Table_065_TurnReception_2.Column01 <> 0) AND (dbo.Table_060_ChequeStatus.Column15 = 3) AND (dbo.Table_035_ReceiptCheques.Column42 = N'" + Class_BasicOperation._UserName + "') AND (Table_065_TurnReception_2.Column16 = N'" + Class_BasicOperation._UserName + "')");
            }
                ...
                if (gridEX1.GetCheckedRows().Length > 0)
                {
                    ...
                    if (MessageBox.Show("آیا مایل به صدور سند حسابداری هستید؟", "توجه", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        if (check.Checked == true)
                        {
                            ...
                            string BedPerson = (gridEX1.GetValue("Column07").ToString().Trim() == "" ? "NULL" : gridEX1.GetValue("Column07").ToString());
                            CommandText = "";
                            foreach (GridEXRow item in gridEX1.GetCheckedRows())
                            {
                                ...
                                /// if (CheckNotExported(int.Parse(item.Cells["ChqID"].Value.ToString()), item.Cells["LastState"].Value.ToString()))
                                {
                                    ...
                                    BedPerson = "NULL";
                                    BedProject = "NULL";
                                    BedPerson = (item.Cells["Column07"].Value.ToString().Trim() == "" ? "NULL" : item.Cells["Column07"].Value.ToString());
                                    ...
                                    //Bes Person
                                    if (check.Checked == true && item.Cells["Column07"].Text.Trim() != "")
                                ...
                                if (_ShowMsg)
                                {
                                    ...
                                    mlt_Bed.Value = null;
                                    mlt_Bes.Value = null;
                                    Frm_05_Return_checks_Load(sender, e);
                                ...
                                if (DocID == 0)
                                    ...
                                    else
                                    {
                                      ...
                                      BedProject, item.Cells["Sharh"].Text.Trim(),
                                       ...
                                       Int16.Parse(item.Cells["Column21"].Value.ToString()),
                                       Int16.Parse(mlt_status.Value.ToString()), _TurnID, Class_BasicOperation._UserName,
                                       Convert.ToDouble(item.Cells["Column22"].Value.ToString()), time, faDatePicker1.Text);
                                    }
                                    ...
                                    if (item.Cells["Column20"].Value.ToString() == "False")
                                    {
                                        CommandText = CommandText + (ExportDoc_Detail(DocID, Bes, Int16.Parse(_Bes_Info[0].ToString()), _Bes_Info[1].ToString(), _Bes_Info[2].ToString(), _Bes_Info[3].ToString(),
                                            _Bes_Info[4].ToString(), BesPerson, BesCenter, BesProject, item.Cells["Sharh"].Text.Trim(), 0,
                                            Convert.ToInt64(Convert.ToDouble(item.Cells["Column05"].Value.ToString())), 0, 0, -1,
                                            Int16.Parse(mlt_status.Value.ToString()), _TurnID, Class_BasicOperation._UserName, 0, time, faDatePicker1.Text));
                                    }
                                    else
                                    {
                                        CommandText = CommandText + ExportDoc_Detail(DocID, Bes, Int16.Parse(_Bes_Info[0].ToString()), _Bes_Info[1].ToString(), _Bes_Info[2].ToString(), _Bes_Info[3].ToString(),
...
int SanadID, string Center, string Project, string User, string Description, string Currency, string CurrencyType, Double CurrencyValue)
    ...
    Int64 Bes, Double CurBed, Double CurBes, Int16 CurType, Int16 SanadType, int Ref, string RegUser, Double CurPrice, object Period, object EffDate)
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                if (UserScope.CheckScope(Class_BasicOperation._UserName, "Column44", 103))
                {
